...
set picked to choose from list {"CL Show Control", "CL Arrangement Builder", "Paradis Latin AutoScene", "CL MIDI Console Monitor"} with title "Installer la Suite CL" with prompt "Sélectionnez les composants à installer :" default items {"CL Show Control", "CL Arrangement Builder", "Paradis Latin AutoScene", "CL MIDI Console Monitor"} with multiple selections allowed
if picked is false then return ""
...
APPLESCRIPT
  ...
  [[ -n "$selected" ]] || exit 0
  components=""
  [[ "$selected" == *"CL Show Control"* ]] && components="remote"
